...
Current next item:
- `[~]` Expand explicit fetch cycle-class checks across remaining ARM/Thumb timing benches beyond the base, literal-load, ARM single-transfer, ARM halfword, ARM swap, and ARM swap-wait timing benches, and compare multiply timing against ARM7TDMI reference behavior
