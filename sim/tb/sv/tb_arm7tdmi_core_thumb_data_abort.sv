...
module tb_arm7tdmi_core_thumb_data_abort
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  assign bus_abort = bus_valid && !bus_write && (bus_addr == 32'h0000_0044) && debug_pc == 32'h0000_0022;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 100; cycle++) begin
      ...
      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "thumb data abort saw invalid bus size");
      end
      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "thumb data abort saw invalid cycle class");
      end
    ...
    if (lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "expected one Thumb data-abort context write/read, saw lr=%0d cpsr=%0d spsr=%0d",
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb data-abort vector loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_data_abort passed");
    $finish;
  end
endmodule
