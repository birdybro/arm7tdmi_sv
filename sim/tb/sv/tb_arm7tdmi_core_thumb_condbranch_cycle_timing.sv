...
module tb_arm7tdmi_core_thumb_condbranch_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 120; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "thumb conditional branch timing test should not write memory");
      end
      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "thumb conditional branch timing saw invalid bus size");
      end
      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "thumb conditional branch timing saw invalid cycle class");
      end
    ...
    if (fetch_20 < 0 || fetch_22 < 0 || fetch_24 < 0 || fetch_28 < 0 ||
        fetch_2a < 0 || fetch_2c < 0) begin
      $fatal(1, "missing Thumb conditional branch timing fetch timestamps");
    end
    if ((fetch_22 - fetch_20) != 2 || (fetch_24 - fetch_22) != 2 ||
        (fetch_28 - fetch_24) != 2 || (fetch_2a - fetch_28) != 2 ||
        (fetch_2c - fetch_2a) != 2) begin
      $fatal(1, "Thumb conditional branch sequence should keep 2-cycle fetch spacing, saw %0d %0d %0d %0d %0d",
             fetch_22 - fetch_20, fetch_24 - fetch_22, fetch_28 - fetch_24,
             fetch_2a - fetch_28, fetch_2c - fetch_2a);
    end
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb conditional branch timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_condbranch_cycle_timing passed");
    $finish;
  end
endmodule
