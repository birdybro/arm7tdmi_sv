...
module tb_arm7tdmi_core_thumb_ls_reg_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
    ...
    if (bus_valid && bus_write) begin
      if (bus_addr == 32'h0000_0044) begin
        ...
        if (bus_wdata !== 32'h0000_002A) begin
          $fatal(1, "Thumb register STRB expected byte 0x2a, got data=%08x", bus_wdata);
        end
        byte_slot <= bus_wdata[7:0];
        byte_store_seen <= byte_store_seen + 1;
      end else if (bus_addr == 32'h0000_0046 && bus_size == BUS_SIZE_HALF) begin
        if (bus_wdata !== 32'h0000_002A) begin
          $fatal(1, "Thumb register STRH expected halfword 0x2a, got data=%08x", bus_wdata);
        end
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 220; cycle++) begin
      @(posedge clk);
      #1;
      sim_cycle++;

      if (bus_valid && !(bus_size inside {BUS_SIZE_BYTE, BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "Thumb register load/store timing saw invalid bus size");
      end
      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "Thumb register load/store timing saw invalid cycle class");
      end
      ...
      if (bus_valid) begin
        ...
        if (bus_addr == 32'h0000_002A && fetch_2a < 0) fetch_2a = sim_cycle;
        if (bus_addr == 32'h0000_002C && fetch_2c < 0) fetch_2c = sim_cycle;
        if (bus_addr == 32'h0000_002E && fetch_2e < 0) fetch_2e = sim_cycle;
        if (bus_addr == 32'h0000_0030 && fetch_30 < 0) fetch_30 = sim_cycle;
        if (bus_addr == 32'h0000_0032 && fetch_32 < 0) fetch_32 = sim_cycle;
        if (bus_addr == 32'h0000_0034 && fetch_34 < 0) fetch_34 = sim_cycle;
        if (bus_addr == 32'h0000_0036 && fetch_36 < 0) fetch_36 = sim_cycle;
        if (bus_addr == 32'h0000_0038 && fetch_38 < 0) fetch_38 = sim_cycle;
        if (bus_addr == 32'h0000_003A && fetch_3a < 0) fetch_3a = sim_cycle;
    ...
    if (fetch_28 < 0 || fetch_2a < 0 || fetch_2c < 0 || fetch_2e < 0 ||
        fetch_30 < 0 || fetch_32 < 0 || fetch_34 < 0 || fetch_36 < 0 ||
        fetch_38 < 0 || fetch_3a < 0) begin
      $fatal(1, "missing Thumb register load/store timing fetch timestamps");
    end
    if ((fetch_2a - fetch_28) != 3 || (fetch_2c - fetch_2a) != 3 ||
        (fetch_2e - fetch_2c) != 3 || (fetch_30 - fetch_2e) != 3 ||
        (fetch_32 - fetch_30) != 3 || (fetch_34 - fetch_32) != 3 ||
        (fetch_36 - fetch_34) != 2 || (fetch_38 - fetch_36) != 3 ||
        (fetch_3a - fetch_38) != 3) begin
      $fatal(1, "unexpected Thumb register LS fetch spacing %0d %0d %0d %0d %0d %0d %0d %0d %0d",
             fetch_2a - fetch_28, fetch_2c - fetch_2a, fetch_2e - fetch_2c,
             fetch_30 - fetch_2e, fetch_32 - fetch_30, fetch_34 - fetch_32,
             fetch_36 - fetch_34, fetch_38 - fetch_36, fetch_3a - fetch_38);
    end
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb register load/store timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_ls_reg_cycle_timing passed");
    $finish;
  end
endmodule
