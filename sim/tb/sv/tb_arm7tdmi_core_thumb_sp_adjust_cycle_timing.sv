...
module tb_arm7tdmi_core_thumb_sp_adjust_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 100; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "Thumb SP adjust timing should not write memory");
      end

      if (bus_valid) begin
        if (!(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
          $fatal(1, "Thumb SP adjust timing saw invalid bus size");
        end
        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
          $fatal(1, "Thumb SP adjust timing saw invalid cycle class");
        end
    ...
    if (fetch_20 < 0 || fetch_22 < 0 || fetch_24 < 0) begin
      $fatal(1, "missing Thumb SP adjust fetch timestamps");
    end
    if ((fetch_22 - fetch_20) != 2 || (fetch_24 - fetch_22) != 2) begin
      $fatal(1, "unexpected Thumb SP adjust fetch spacing %0d %0d",
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb SP adjust timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_sp_adjust_cycle_timing passed");
    $finish;
  end
endmodule
