...
module tb_arm7tdmi_core_thumb_swi_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 80; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "Thumb SWI timing should not write memory");
      end

      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "Thumb SWI timing saw invalid bus size");
      end

      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "Thumb SWI timing saw invalid bus cycle");
      end
      ...
      if (retired && debug_pc == 32'h0000_0010 && debug_cpsr == 32'h0000_00D3) begin
        loop_seen++;
      end
    end

    if ((fetch_20 < 0) || (fetch_08 < 0) || (fetch_0c < 0) || (fetch_10 < 0)) begin
      $fatal(1, "missing Thumb SWI timing fetch timestamps");
    end

    if ((fetch_08 - fetch_20) != 3) begin
      $fatal(1, "Thumb SWI should incur one exception-save internal cycle before vector fetch, saw %0d",
             fetch_08 - fetch_20);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible Thumb SWI internal cycle, saw %0d", int_cycles_seen);
    end

    if (lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected Thumb SWI timing observations lr=%0d cpsr=%0d spsr=%0d",
    ...
    $display("tb_arm7tdmi_core_thumb_swi_cycle_timing passed");
    $finish;
  end
endmodule
