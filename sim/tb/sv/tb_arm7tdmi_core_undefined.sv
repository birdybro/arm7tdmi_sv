...
module tb_arm7tdmi_core_undefined
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 50; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "undefined smoke should not write memory");
      end

      if (bus_valid && bus_size !== BUS_SIZE_WORD) begin
        $fatal(1, "undefined smoke expected word fetches");
      end

      if (bus_valid && bus_cycle !== BUS_CYCLE_NONSEQ && bus_cycle !== BUS_CYCLE_SEQ) begin
        $fatal(1, "undefined smoke saw invalid bus cycle");
      end
    ...
    if (lr_seen != 1) begin
      $fatal(1, "expected one undefined LR write of 0x4, saw %0d", lr_seen);
    end

    if (cpsr_seen != 1) begin
      $fatal(1, "expected one MRS CPSR after undefined exception, saw %0d", cpsr_seen);
    end

    if (spsr_seen != 1) begin
      $fatal(1, "expected one MRS SPSR after undefined exception, saw %0d", spsr_seen);
    end

    if (debug_cpsr !== 32'h0000_00DB) begin
      $fatal(1, "expected undefined exception to enter UND mode, got %08x", debug_cpsr);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected undefined vector loop to retire at least twice, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_undefined passed");
    $finish;
  end
endmodule
