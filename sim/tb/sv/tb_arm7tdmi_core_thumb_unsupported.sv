...
module tb_arm7tdmi_core_thumb_unsupported
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  task automatic check_bus_contract;
    if (bus_write || bus_wdata !== 32'h0000_0000) begin
      $fatal(1, "thumb unsupported test should not write memory");
    end

    if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
      $fatal(1, "thumb unsupported saw invalid bus size");
    end

    if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
      $fatal(1, "thumb unsupported saw invalid cycle class");
    end
  ...
  initial begin
    ...
    if (lr_seen != 1) begin
      $fatal(1, "expected one Thumb unsupported LR write of 0x22, saw %0d", lr_seen);
    end

    if (cpsr_seen != 1) begin
      $fatal(1, "expected one MRS CPSR after Thumb unsupported, saw %0d", cpsr_seen);
    end

    if (spsr_seen != 1) begin
      $fatal(1, "expected one MRS SPSR after Thumb unsupported, saw %0d", spsr_seen);
    end

    if (debug_cpsr !== 32'h0000_00DB) begin
      $fatal(1, "expected Thumb unsupported to enter UND mode, got %08x", debug_cpsr);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb unsupported vector loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_unsupported passed");
    $finish;
  end
endmodule
