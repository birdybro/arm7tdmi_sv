...
module tb_arm7tdmi_core_interrupt_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  task automatic run_case(
    ...
    for (int cycle = 0; cycle < 80; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "interrupt timing test should not write memory");
      end

      if (bus_valid) begin
        if (bus_size != BUS_SIZE_WORD) begin
          $fatal(1, "interrupt timing expected word fetches");
        end

        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
          $fatal(1, "interrupt timing bus_valid expected seq/nonseq, got %0d", bus_cycle);
        end
    ...
    if (fetch_vector < 0) begin
      $fatal(1, "missing interrupt timing vector fetch");
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible interrupt internal cycle, saw %0d", int_cycles_seen);
    end

    if (lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected interrupt timing observations lr=%0d cpsr=%0d spsr=%0d",
             lr_seen, cpsr_seen, spsr_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected interrupt timing loop to retire at least twice, saw %0d", loop_seen);
    end
  ...
  initial begin
    ...
    run_case(1'b0, 32'h0000_0092, 32'h0000_0018, 32'h0000_0020);
    run_case(1'b1, 32'h0000_00D1, 32'h0000_001C, 32'h0000_0024);

    $display("tb_arm7tdmi_core_interrupt_cycle_timing passed");
    $finish;
  end
endmodule
