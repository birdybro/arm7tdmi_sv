...
module tb_arm7tdmi_core_exception_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 70; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "exception timing test should not write memory");
      end

      if (bus_valid) begin
        if (bus_size != BUS_SIZE_WORD) begin
          $fatal(1, "exception timing expected word fetches");
        end

        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
          $fatal(1, "exception timing bus_valid expected seq/nonseq, got %0d", bus_cycle);
        end
    ...
    if ((fetch_04 < 0) || (fetch_08 < 0) || (fetch_0c < 0) || (fetch_10 < 0)) begin
      $fatal(1, "missing exception timing fetch timestamps");
    end

    if ((fetch_08 - fetch_04) != 3) begin
      $fatal(1, "SWI should incur one exception-save internal cycle before vector fetch, saw %0d",
             fetch_08 - fetch_04);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible exception-save internal cycle, saw %0d", int_cycles_seen);
    end

    if (lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected exception timing observations lr=%0d cpsr=%0d spsr=%0d",
             lr_seen, cpsr_seen, spsr_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected exception timing loop to retire at least twice, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_exception_cycle_timing passed");
    $finish;
  end
endmodule
