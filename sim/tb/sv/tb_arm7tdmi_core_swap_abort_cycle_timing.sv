...
module tb_arm7tdmi_core_swap_abort_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_ff @(posedge clk) begin
    if (bus_valid && bus_write) begin
      if (bus_addr !== 32'h0000_0080 || bus_size !== BUS_SIZE_WORD ||
          bus_wdata !== 32'h0000_002A || !bus_abort) begin
        $fatal(1, "unexpected swap-abort-cycle write addr=%08x size=%0d data=%08x abort=%0b",
               bus_addr, bus_size, bus_wdata, bus_abort);
      end
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 90; cycle++) begin
      ...
      if (bus_valid) begin
        if (!(bus_size inside {BUS_SIZE_WORD})) begin
          $fatal(1, "swap abort timing expected word bus size");
        end
        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
          $fatal(1, "swap abort timing bus_valid expected seq/nonseq, got %0d", bus_cycle);
        end
    ...
    if ((fetch_0c < 0) || (fetch_10 < 0) || (fetch_14 < 0) || (fetch_18 < 0)) begin
      $fatal(1, "missing swap abort timing fetch timestamps");
    end

    if ((fetch_10 - fetch_0c) != 5) begin
      $fatal(1, "swap abort should redirect after the full two-phase swap abort path in current timing mode, saw %0d",
             fetch_10 - fetch_0c);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible swap abort internal cycle, saw %0d", int_cycles_seen);
    end

    if (base_seen != 1 || value_seen != 1 || aborted_store_seen != 1 || old_value_seen != 0 ||
        lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected swap abort timing observations base=%0d value=%0d store=%0d old=%0d lr=%0d cpsr=%0d spsr=%0d",
             base_seen, value_seen, aborted_store_seen, old_value_seen, lr_seen, cpsr_seen, spsr_seen);
    end
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected swap-abort timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_swap_abort_cycle_timing passed");
    $finish;
  end
endmodule
