...
module tb_arm7tdmi_core_coprocessor_abort_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 90; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "coprocessor abort timing test should not write memory");
      end

      if (bus_valid) begin
        if (!(bus_size inside {BUS_SIZE_WORD})) begin
          $fatal(1, "coprocessor abort timing expected word bus accesses only");
        end

        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ, BUS_CYCLE_COPROC})) begin
          $fatal(1, "coprocessor abort timing saw invalid cycle class %0d", bus_cycle);
        end
        ...
        if (bus_addr == 32'h0000_0040) begin
          if (bus_cycle != BUS_CYCLE_COPROC) begin
            $fatal(1, "aborted LDC beat should use coprocessor cycle, saw %0d", bus_cycle);
          end
          coproc_mem_cycles++;
        end
      ...
      if (coproc_valid && coproc_ready && coproc_accept) begin
        coproc_handshakes++;
      end
    ...
    if ((fetch_08 < 0) || (fetch_10 < 0) || (fetch_14 < 0) || (fetch_18 < 0)) begin
      $fatal(1, "missing coprocessor abort timing fetch timestamps");
    end

    if ((fetch_10 - fetch_08) != 4) begin
      $fatal(1, "coprocessor LDC abort should redirect after the aborted coprocessor memory beat, saw spacing %0d",
             fetch_10 - fetch_08);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible coprocessor abort internal cycle, saw %0d", int_cycles_seen);
    end

    if (coproc_mem_cycles != 1) begin
      $fatal(1, "expected one aborted coprocessor memory beat, saw %0d", coproc_mem_cycles);
    end

    if (coproc_handshakes != 0) begin
      $fatal(1, "aborted first-beat LDC should not reach a coprocessor handshake, saw %0d", coproc_handshakes);
    end

    if (base_setup_seen != 1 || aborted_ldc_wb_seen != 1 || lr_seen != 1 ||
        cpsr_seen != 1 || spsr_seen != 1 || unexpected_gpr_seen != 0) begin
      $fatal(1, "unexpected coprocessor abort timing observations base=%0d wb=%0d lr=%0d cpsr=%0d spsr=%0d gpr=%0d",
             base_setup_seen, aborted_ldc_wb_seen, lr_seen, cpsr_seen, spsr_seen,
             unexpected_gpr_seen);
    end

    if (cp_regs[4] !== 32'h0000_0000 || cp_regs[5] !== 32'h0000_0000) begin
      $fatal(1, "aborted LDC should not update coprocessor registers c4=%08x c5=%08x",
             cp_regs[4], cp_regs[5]);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected coprocessor abort timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_coprocessor_abort_cycle_timing passed");
    $finish;
  end
endmodule
