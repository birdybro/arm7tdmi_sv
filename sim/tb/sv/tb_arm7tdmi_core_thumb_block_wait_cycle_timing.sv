...
module tb_arm7tdmi_core_thumb_block_wait_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_ff @(posedge clk) begin
    if (bus_valid && !bus_ready) begin
      if (stall_active && (bus_addr !== stall_addr_q || bus_wdata !== stall_wdata_q)) begin
        $fatal(1, "Thumb block timing changed while stalled addr=%08x/%08x data=%08x/%08x",
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 180; cycle++) begin
      ...
      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "Thumb block wait timing saw invalid bus size");
      end
      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "Thumb block wait timing saw invalid cycle class");
      end
    ...
    if ((fetch_22 < 0) || (fetch_24 < 0)) begin
      $fatal(1, "missing Thumb block wait timing fetch timestamps");
    end
    if ((fetch_24 - fetch_22) != 9) begin
      $fatal(1, "waited Thumb LDMIA with 3 beats and writeback should take 9 cycles to next fetch, saw %0d",
             fetch_24 - fetch_22);
    end
    ...
    if (int_cycles_seen < 2) begin
      $fatal(1, "expected visible Thumb block writeback internal cycles, saw %0d", int_cycles_seen);
    end
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb block wait timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_block_wait_cycle_timing passed");
    $finish;
  end
endmodule
