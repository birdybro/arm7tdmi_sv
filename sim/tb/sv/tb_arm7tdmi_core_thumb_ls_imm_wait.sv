...
module tb_arm7tdmi_core_thumb_ls_imm_wait
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_ff @(posedge clk) begin
    if (bus_valid && !bus_ready) begin
      if (stall_active && (bus_write !== stall_write_q || bus_wdata !== stall_wdata_q)) begin
        $fatal(1, "thumb LS immediate changed while stalled write=%0d/%0d data=%08x/%08x",
    ...
    if (bus_valid && bus_ready && bus_write) begin
      if (bus_addr !== 32'h0000_0044 || bus_size !== BUS_SIZE_WORD ||
        ...
        $fatal(1, "unexpected waited Thumb STR addr=%08x size=%0d data=%08x",
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 120; cycle++) begin
      ...
      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "Thumb LS immediate wait saw invalid bus size");
      end
      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "Thumb LS immediate wait saw invalid cycle class");
      end
    ...
    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb LS immediate wait loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_ls_imm_wait passed");
    $finish;
  end
endmodule
