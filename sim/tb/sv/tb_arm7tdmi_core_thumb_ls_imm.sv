...
module tb_arm7tdmi_core_thumb_ls_imm
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_comb begin
    unique case (bus_addr)
      ...
      32'h0000_0028: bus_rdata = 32'h0000_7141; // Thumb STRB r1, [r0, #5]
      32'h0000_002A: bus_rdata = 32'h0000_7943; // Thumb LDRB r3, [r0, #5]
      32'h0000_002C: bus_rdata = 32'h0000_80C1; // Thumb STRH r1, [r0, #6]
      32'h0000_002E: bus_rdata = 32'h0000_88C4; // Thumb LDRH r4, [r0, #6]
      32'h0000_0030: bus_rdata = 32'h0000_E7FE; // Thumb B .
      32'h0000_0044: bus_rdata = data_word;
      32'h0000_0045: bus_rdata = {24'h0, data_word[15:8]};
      32'h0000_0046: bus_rdata = {16'h0, data_word[31:16]};
    ...
    if (bus_valid && bus_write) begin
      if (bus_addr == 32'h0000_0044) begin
        if (bus_size !== BUS_SIZE_WORD || bus_wdata !== 32'h0000_002A) begin
          $fatal(1, "Thumb STR expected word 0x2a, got size=%0d data=%08x", bus_size, bus_wdata);
        end
        ...
        if (bus_size !== BUS_SIZE_BYTE || bus_wdata !== 32'h0000_002A) begin
          $fatal(1, "Thumb STRB expected byte 0x2a, got size=%0d data=%08x", bus_size, bus_wdata);
        end

        data_word <= {data_word[31:16], bus_wdata[7:0], data_word[7:0]};
        byte_store_seen <= byte_store_seen + 1;
      end else if (bus_addr == 32'h0000_0046) begin
        if (bus_size !== BUS_SIZE_HALF || bus_wdata !== 32'h0000_002A) begin
          $fatal(1, "Thumb STRH expected halfword 0x2a, got size=%0d data=%08x", bus_size, bus_wdata);
        end

        data_word <= {bus_wdata[15:0], data_word[15:0]};
        half_store_seen <= half_store_seen + 1;
      end else begin
        $fatal(1, "unexpected Thumb store address %08x", bus_addr);
      end
    end
  end
  ...
  task automatic check_bus_contract;
    if (bus_valid && !(bus_size inside {BUS_SIZE_BYTE, BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
      $fatal(1, "Thumb immediate load/store saw invalid bus size");
    end

    if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
      $fatal(1, "Thumb immediate load/store saw invalid cycle class");
    end
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 140; cycle++) begin
      ...
      if (retired && debug_pc == 32'h0000_0030 && debug_cpsr[5]) begin
        loop_seen++;
      end
    end
    ...
    if (word_store_seen != 1 || word_load_seen != 1 ||
        ...
        half_store_seen != 1 || half_load_seen != 1) begin
      $fatal(1, "expected one Thumb immediate LS path, saw sw=%0d lw=%0d sb=%0d lb=%0d sh=%0d lh=%0d",
             word_store_seen, word_load_seen, byte_store_seen, byte_load_seen,
             half_store_seen, half_load_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb immediate load/store loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_ls_imm passed");
    $finish;
  end
endmodule
