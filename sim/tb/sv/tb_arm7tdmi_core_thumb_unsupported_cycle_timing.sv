...
module tb_arm7tdmi_core_thumb_unsupported_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_comb begin
    unique case (bus_addr)
      ...
      32'h0000_0020: bus_rdata = 32'h0000_C000; // Thumb STMIA r0!, {}
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 80; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "Thumb unsupported timing should not write memory");
      end

      if (bus_valid && !(bus_size inside {BUS_SIZE_HALF, BUS_SIZE_WORD})) begin
        $fatal(1, "Thumb unsupported timing saw invalid bus size");
      end

      if (bus_valid && !(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
        $fatal(1, "Thumb unsupported timing saw invalid bus cycle");
      end
    ...
    if ((fetch_20 < 0) || (fetch_04 < 0) || (fetch_08 < 0) || (fetch_0c < 0)) begin
      $fatal(1, "missing Thumb unsupported timing fetch timestamps");
    end

    if ((fetch_04 - fetch_20) != 3) begin
      $fatal(1, "Thumb unsupported should incur one exception-save internal cycle before vector fetch, saw %0d",
             fetch_04 - fetch_20);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible Thumb unsupported internal cycle, saw %0d", int_cycles_seen);
    end

    if (lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected Thumb unsupported timing observations lr=%0d cpsr=%0d spsr=%0d",
             lr_seen, cpsr_seen, spsr_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected Thumb unsupported vector loop to retire at least twice, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_thumb_unsupported_cycle_timing passed");
    $finish;
  end
endmodule
