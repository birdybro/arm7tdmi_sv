...
module tb_arm7tdmi_core_block_abort_cycle_timing
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 90; cycle++) begin
      ...
      if (bus_write || bus_wdata !== 32'h0000_0000) begin
        $fatal(1, "block abort timing test should not write memory");
      end

      if (bus_valid) begin
        if (bus_size !== BUS_SIZE_WORD) begin
          $fatal(1, "block abort timing expected word transfers");
        end
        if (!(bus_cycle inside {BUS_CYCLE_NONSEQ, BUS_CYCLE_SEQ})) begin
          $fatal(1, "block abort timing bus_valid expected seq/nonseq, got %0d", bus_cycle);
        end
    ...
    if ((fetch_08 < 0) || (fetch_10 < 0) || (fetch_14 < 0) || (fetch_18 < 0)) begin
      $fatal(1, "missing block abort timing fetch timestamps");
    end

    if ((fetch_10 - fetch_08) != 5) begin
      $fatal(1, "block abort should redirect after the multi-beat load abort path in current timing mode, saw %0d",
             fetch_10 - fetch_08);
    end

    if (int_cycles_seen < 1) begin
      $fatal(1, "expected visible block abort internal cycle, saw %0d", int_cycles_seen);
    end

    if (base_seen != 1 || first_load_seen != 1 || aborted_load_seen != 0 || later_load_seen != 0 ||
        base_wb_seen != 0 || lr_seen != 1 || cpsr_seen != 1 || spsr_seen != 1) begin
      $fatal(1, "unexpected block abort timing observations base=%0d first=%0d aborted=%0d later=%0d wb=%0d lr=%0d cpsr=%0d spsr=%0d",
             base_seen, first_load_seen, aborted_load_seen, later_load_seen,
             base_wb_seen, lr_seen, cpsr_seen, spsr_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected block-abort timing loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_block_abort_cycle_timing passed");
    $finish;
  end
endmodule
