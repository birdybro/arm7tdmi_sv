...
module tb_arm7tdmi_core_coprocessor
  import arm7tdmi_pkg::*;
;
  logic clk;
  logic rst_n;
  ...
  always_ff @(posedge clk) begin
    if (bus_valid && bus_write && bus_cycle == BUS_CYCLE_COPROC) begin
      unique case (bus_addr)
        ...
        default:       $fatal(1, "unexpected coprocessor store addr=%08x data=%08x",
                              bus_addr, bus_wdata);
      endcase
    end
  end
  ...
  initial begin
    ...
    for (int cycle = 0; cycle < 80; cycle++) begin
      ...
      if (bus_valid && (bus_addr inside {32'h0000_0040, 32'h0000_0044, 32'h0000_0050, 32'h0000_0054})) begin
        if (bus_size != BUS_SIZE_WORD) begin
          $fatal(1, "coprocessor transfers should stay word-sized, saw %0d", bus_size);
        end
        if (bus_cycle != BUS_CYCLE_COPROC) begin
          $fatal(1, "coprocessor transfers should use coprocessor bus cycles, saw %0d", bus_cycle);
        end
        coproc_cycle_seen++;
      end
      ...
      if (bus_valid && bus_write && bus_cycle == BUS_CYCLE_COPROC &&
    ...
    if (mrc_seen < 1) begin
      $fatal(1, "expected at least one MRC readback of 0x55, saw %0d", mrc_seen);
    end

    if (cdp_seen < 1) begin
      $fatal(1, "expected at least one CDP-derived MRC value of 0x56, saw %0d", cdp_seen);
    end

    if (ldc_wb_seen != 1 || stc_wb_seen != 1) begin
      $fatal(1, "expected one LDC/STC writeback each, saw ldc=%0d stc=%0d",
             ldc_wb_seen, stc_wb_seen);
    end

    if (stc_store_seen != 2) begin
      $fatal(1, "expected two STC memory writes, saw %0d", stc_store_seen);
    end

    if (cp_regs[4] !== 32'h1122_3344 || cp_regs[5] !== 32'h5566_7788) begin
      $fatal(1, "unexpected LDC payload c4=%08x c5=%08x", cp_regs[4], cp_regs[5]);
    end

    if (data_word2 !== 32'h1122_3344 || data_word3 !== 32'h5566_7788) begin
      $fatal(1, "unexpected STC payload word2=%08x word3=%08x", data_word2, data_word3);
    end

    if (coproc_cycle_seen < 4) begin
      $fatal(1, "expected coprocessor bus cycles for both LDC/STC beats, saw %0d", coproc_cycle_seen);
    end

    if (loop_seen < 2) begin
      $fatal(1, "expected coprocessor loop to retire, saw %0d", loop_seen);
    end

    $display("tb_arm7tdmi_core_coprocessor passed");
    $finish;
  end
endmodule
