...
- Initial processor-level co-simulation harness: generic RTL retire trace bench, JSON trace compare script, scripted MAME trace render/normalization flow, and smoke images covering ARM branch/memory/block cases plus growing Thumb interwork, interrupt/prefetch-abort entry, control-flow, ALU opcode, high-register, shift, add/sub, stack, SP-relative, address-generation, and load/store coverage.
